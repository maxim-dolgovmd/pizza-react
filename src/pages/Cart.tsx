...
import {setItem, setClear, setRemoveItem, setMinusItem, selectCart, CartItem} from '../redux/slices/cartSlice'
import { useSelector,  useDispatch} from "react-redux"
import { Link } from 'react-router-dom'
import cartEmptyImage from '../../src/assets/empty-cart.png'

const Cart: React.FC = () => {
  const dispatch = useDispatch()
  const {items, sumCart, sumCount} = useSelector(selectCart)
  console.log(items)

  const onClickPlus = (idObj: CartItem) => {
    ...
    <div>
      ...
      <div className="cart__top">
        ...
        <div className="cart__clear">
          ...
          <span onClick={() => dispatch(setClear())}>Очистить корзину</span>
        </div>
      </div>
      <div className="cart__items">
        { 
          ...
          <div className="cart__item-info">
            <h3>{obj.title}</h3>
            <p>{obj.type} тесто, {obj.sizes} см.</p>
          </div>
          <div className="cart__item-count">
            <button 
              disabled={obj.count === 1}
              className={obj.count === 1 ? "button button--outline button--circle cart__item-count-minus cart__item-count-minus--disabled" : "button button--outline button--circle cart__item-count-minus"} 
              onClick={() => dispatch(setMinusItem(obj))}
            >
            ...
            {/* <b>{obj.count === 0 ? onClickRemove(obj) : obj.count}</b> */}
            <b>{obj.count}</b>
            <button className="button button--outline button--circle cart__item-count-plus" onClick={() => onClickPlus(obj)}>
              <svg width="10" height="10" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M5.92001 3.84V5.76V8.64C5.92001 9.17016 5.49017 9.6 4.96001 9.6C4.42985 9.6 4.00001 9.17016 4.00001 8.64L4 5.76L4.00001 3.84V0.96C4.00001 0.42984 4.42985 0 4.96001 0C5.49017 0 5.92001 0.42984 5.92001 0.96V3.84Z" fill="#EB5A1E" />
                <path d="M5.75998 5.92001L3.83998 5.92001L0.959977 5.92001C0.429817 5.92001 -2.29533e-05 5.49017 -2.29301e-05 4.96001C-2.2907e-05 4.42985 0.429817 4.00001 0.959977 4.00001L3.83998 4L5.75998 4.00001L8.63998 4.00001C9.17014 4.00001 9.59998 4.42985 9.59998 4.96001C9.59998 5.49017 9.17014 5.92001 8.63998 5.92001L5.75998 5.92001Z" fill="#EB5A1E" />
              </svg>
      ...
      <div className="cart__bottom">
        <div className="cart__bottom-details">
          <span> Всего пицц: <b>{!sumCount ? 0 : sumCount} шт.</b> </span>
          <span> Сумма заказа: <b>{!sumCart ? 0 : sumCart} ₽</b> </span>
        </div>
